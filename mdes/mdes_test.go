...
func TestEncryptAndDecryptTokenizeRequest(t *testing.T) {
	...
	assert.NoError(t, json.Unmarshal(encrypted, &ePayload))
...
func TestGetTokenAPI(t *testing.T) {
	...
	resp, err := mdesAPI.GetToken("98765432101", "DWSPMC000000000132d72d4fcb2f4136a0532d3093ff1a45")
	assert.NoError(t, err)
...
func TestSearchAPI(t *testing.T) {
	...
	resp, err := mdesAPI.Search("98765432101", "", "", CardAccountData{AccountNumber: "5123456789012345", ExpiryMonth: "09", ExpiryYear: "21", SecurityCode: "123"})
	assert.NoError(t, err)
	log.Printf("Received: \n%+v", resp)
}
...
func TestTRIDnotifyInternal(t *testing.T) {
	_, err := mdesAPI.tridCB([]byte(`{"requestID":"123456789","tokenRequestors":[{"entityId":"123456","TokenRequestorID":"123456789"}]}`))
	assert.NoError(t, err)
}

func TestTRIDnotifyRequest(t *testing.T) {
	time.Sleep(time.Millisecond * 10)
	_, err := request(tridcb, []byte(`{"requestID":"123456789","tokenRequestors":[{"entityId":"123456","TokenRequestorID":"123456789"}]}`))
	assert.NoError(t, err)
}
